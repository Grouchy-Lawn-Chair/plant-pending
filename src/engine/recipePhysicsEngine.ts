...
function randomInside(polygon: PhysicsPoint[], random: () => number, minimumEdge = 0, tries = 480) { const box = bounds(polygon), center = centroid(polygon); for (let i = 0; i < tries; i++) { const point = { x: box.minX + random() * (box.maxX - box.minX), y: box.minY + random() * (box.maxY - box.minY) }; if (pointInPolygon(point, polygon) && edgeDistance(point, polygon) >= minimumEdge) return point; } return pointInPolygon(center, polygon) ? center : polygon[0]; }
...
function runPass(options: RecipePhysicsOptions, seed: number): RecipePhysicsResult {
  ...
  const desired = Math.min(400, estimateCount(options, plants));
...
export function runRecipePhysics(options: RecipePhysicsOptions): RecipePhysicsResult {
  ...
  const explicitTotal = options.plants.reduce((sum, plant) => sum + Math.max(0, Math.round(plant.count ?? 0)), 0);
  if (explicitTotal > 400) throw new Error('Recipe generation is limited to 400 explicitly requested plants per run.');
