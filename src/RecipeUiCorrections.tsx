...
export default function RecipeUiCorrections(){
  useEffect(()=>{
    ...
    const existingZoneIds=new Set((readPlan()?.zones||[]).map(zone=>zone.id));
    const correctedZoneIds=new Set<string>();
    ...
    const correctNewZoneOpacity=(plan:GardenPlan)=>{const newZones=(plan.zones||[]).filter(zone=>!existingZoneIds.has(zone.id)&&!correctedZoneIds.has(zone.id));if(!newZones.length)return;const root=document.getElementById('root');const update=findFunction(findFiber(root),'onUpdateZone') as UpdateZone|null;newZones.forEach(zone=>{correctedZoneIds.add(zone.id);existingZoneIds.add(zone.id);if(Math.abs((zone.opacity??.28)-.28)>.001)return;if(update)update(zone.id,{opacity:.1});else applyPlan({...plan,zones:(plan.zones||[]).map(item=>item.id===zone.id?{...item,opacity:.1}:item),updatedAt:new Date().toISOString()});});};
