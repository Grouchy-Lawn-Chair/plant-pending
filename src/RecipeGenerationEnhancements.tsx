...
type PendingRun = {
  ...
  zone: GardenZone;
  plan: GardenPlan;
  issues: Array<{ plant: PlantCardInfo; availableInches: number }>;
  hedgePlants: PlantCardInfo[];
};
...
function collectPlantCards(host: HTMLElement): PlantCardInfo[] {
  return [...host.querySelectorAll<HTMLElement>('div.rounded-xl')].flatMap(card => {
    ...
    return checkbox && widthInput && name
      ? [{ name, widthInches: Number(widthInput.value) || 0, enabled: checkbox.checked, placement, checkbox, widthInput }]
      : [];
  });
}
function maximumClearDiameter(zone: GardenZone, exclusions: GardenZone[], rocks: PlacedPlant[], ppf: number) {
  ...
  for (let yi = 0; yi <= 36; yi++) {
    for (let xi = 0; xi <= 36; xi++) {
      const point = {
        x: box.minX + xi / 36 * (box.maxX - box.minX),
...
async function cleanGeneratedZone(plan: GardenPlan, zone: GardenZone, progress: (value: number) => void) {
  ...
  for (let index = 0; index < plants.length; index++) {
    ...
    if (best) Object.assign(plant, best);
    else rejected.add(plant.instanceId);
    if (index % 8 === 0) {
      progress(72 + Math.round(index / Math.max(1, plants.length) * 18));
...
export default function RecipeGenerationEnhancements() {
  ...
  useEffect(() => {
    if (!visible) return;
    const timer = window.setInterval(() => setStage(value => value + 1), 1150);
  ...
  useEffect(() => {
    ...
    const run = (button: HTMLButtonElement, zone: GardenZone, beforePlan: GardenPlan) => {
      ...
      setStage(0);
      setVisible(true);
      const stamp = beforePlan.updatedAt || '';
      window.setTimeout(() => {
        bypass = true;
        button.click();
        bypass = false;
        ...
        const poll = window.setInterval(async () => {
          ...
          window.clearInterval(poll);
          setStage(5);
          const freshZone = (next.zones || []).find(item => item.id === zone.id) || zone;
          const improved = await cleanGeneratedZone(next, freshZone, () => setStage(value => Math.max(value, 6)));
          applyPlan(improved);
    ...
    const onClick = (event: MouseEvent) => {
      ...
      const host = button?.closest<HTMLElement>('[data-recipe-react-host]');
      if (!button || !host || bypass || running) return;
      const text = button.textContent?.trim().toLowerCase() || '';
      if (text !== 'generate' && text !== 'new seed + generate') return;
      event.preventDefault();
      event.stopImmediatePropagation();
      const modal = host.closest('div.fixed') || host.parentElement;
      ...
      if (issues.length || hedgePlants.length) {
        setTrimWidths(Object.fromEntries(hedgePlants.map(item => [item.name, item.widthInches])));
        setPending({ button, zone, plan, issues, hedgePlants });
        return;
      }
      run(button, zone, plan);
    };

    const proceedEvent = (event: Event) => {
      const detail = (event as CustomEvent<{ button: HTMLButtonElement; zone: GardenZone; plan: GardenPlan }>).detail;
      run(detail.button, detail.zone, detail.plan);
    };

    document.addEventListener('recipe-preflight-proceed', proceedEvent);
    document.addEventListener('click', onClick, true);
    return () => {
      document.removeEventListener('recipe-preflight-proceed', proceedEvent);
  ...
  const proceed = (mode: 'adjust' | 'exclude' | 'trim' | 'anyway') => {
    ...
    if (mode === 'exclude') pending.issues.forEach(issue => issue.plant.checkbox.click());
    if (mode === 'trim') {
      pending.hedgePlants.forEach(item => setInputValue(item.widthInput, String(trimWidths[item.name] ?? item.widthInches)));
    }
    const detail = { button: pending.button, zone: pending.zone, plan: pending.plan };
    setPending(null);
    window.setTimeout(() => document.dispatchEvent(new CustomEvent('recipe-preflight-proceed', { detail })), 100);
  };
  ...
  return <>
      ...
      <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-sm">
        <div className="max-h-[calc(100vh-32px)] w-[min(620px,100%)] overflow-auto rounded-3xl border border-amber-300/35 bg-slate-950 p-6 text-white shadow-2xl">
          ...
          {pending.issues.length > 0 && (
            <div className="mt-4 rounded-2xl border border-amber-400/30 bg-amber-950/25 p-4">
              <div className="font-bold text-amber-200">Too large at the current width</div>
              {pending.issues.map(issue => (
                <div key={issue.plant.name} className="mt-1 text-sm">• {issue.plant.name}: {Math.round(issue.plant.widthInches)} in; about {Math.round(issue.availableInches)} in fits.</div>
              ))}
            </div>
          )}
          {pending.hedgePlants.length > 0 && (
            <div className="mt-4 rounded-2xl border border-violet-400/30 bg-violet-950/25 p-4">
              <div className="font-bold text-violet-200">Hedge detected. It would like a haircut plan.</div>
              <div className="mt-2 space-y-3">
                {pending.hedgePlants.map(item => (
                  <label key={item.name} className="block text-sm">
                    <span className="font-bold">{item.name}</span>
                    <span className="ml-2 text-slate-400">mature: {item.widthInches} in</span>
                    <input
                      type="number"
                      min="6"
                      max="240"
                      value={trimWidths[item.name] ?? item.widthInches}
                      onChange={event => setTrimWidths(value => ({ ...value, [item.name]: Number(event.target.value) || 6 }))}
                      className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2"
                    />
                  </label>
                ))}
              </div>
              <button type="button" onClick={() => proceed('trim')} className="mt-3 w-full rounded-xl bg-violet-600 px-3 py-2.5 font-bold">Use these maintained widths and generate</button>
            </div>
          )}
          <div className="mt-4 grid gap-2 sm:grid-cols-2">
            {pending.issues.length > 0 && <button onClick={() => proceed('exclude')} className="rounded-xl bg-amber-600 px-3 py-2.5 font-bold">Exclude plants that cannot fit</button>}
            <button onClick={() => proceed('adjust')} className="rounded-xl border border-slate-600 bg-slate-900 px-3 py-2.5 font-bold">Go back and adjust widths</button>
            <button onClick={() => proceed('anyway')} className="rounded-xl border border-rose-500/50 bg-rose-950/40 px-3 py-2.5 font-bold">Generate anyway, tempt fate</button>
          </div>
        </div>
      </div>,
      document.body,
    ...
    {visible && createPortal(
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/55 backdrop-blur-sm">
        <style>{`@keyframes shrub-slide{0%{transform:translateX(-100%)}100%{transform:translateX(400%)}}`}</style>
        <div className="w-[min(560px,calc(100vw-32px))] rounded-3xl border border-violet-300/30 bg-slate-950 p-6 shadow-2xl">
          <div className="text-[11px] font-black uppercase tracking-[.2em] text-violet-300">Botanical machinery in progress</div>
          <div className="mt-2 text-xl font-black text-white">Please remain calm. The plants are being negotiated with.</div>
          <div className="mt-4 min-h-14 rounded-2xl border border-slate-700 bg-slate-900/80 p-4 text-sm text-slate-200">{rotatingMessage}</div>
          <div className="relative mt-5 h-4 overflow-hidden rounded-full border border-slate-700 bg-slate-900">
            <div className="absolute inset-y-0 w-1/3 rounded-full bg-violet-400" style={{ animation: 'shrub-slide 1.4s linear infinite' }} />
          </div>
          <div className="mt-2 flex justify-between text-xs text-slate-400"><span>Still shrubbin’…</span><span>Probably needs a shrub.</span></div>
        </div>
      </div>,
      document.body,
    )}
